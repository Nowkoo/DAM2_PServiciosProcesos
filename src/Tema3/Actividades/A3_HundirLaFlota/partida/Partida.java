package Tema3.Actividades.A3_HundirLaFlota.partida;
...
public class Partida {
    	...
    	if(estado == AGUA || estado == TOCADO || estado == HUNDIDO) {
	    	...
	    	if(barco.tocaBarco()) {
	    		...
	    		if(barco.getOrientacion() == 'H') {
	    			for(int i = columna; i < columna + tamanyo; i++) {
	    				mar[fila][i] = HUNDIDO;
	    			}
	    		} else {
	    			for(int i = fila; i < fila + tamanyo; i++) {
	    				mar[i][columna] = HUNDIDO;
	    			}
	    		}
	    		return estado;
	    	} else {
	...
	public String[] getSolucion() {
        String[] infoBarcos = new String[barcos.size()];
        for(int i = 0; i<barcos.size(); i++) {
        	infoBarcos[i] = this.getBarco(i);
        }
		return infoBarcos;
	}
